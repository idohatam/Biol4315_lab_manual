---
title: 'Biol 4315 Lab 1: Intro to RMarkdown, GitHub, and Computational Environment Setup'
author: "Dr. Ido Hatam"
...
> _Goal: Configure your lab computer and install required tools using the course R package._
...
> _Goal: Set up a GitHub account and connect it to your local git client._
...
```{bash check git, class.source='klippy'}
git -h
...
```{bash firstgit, eval=F, class.source='klippy'}
...
> _Goal: Connect RStudio to GitHub and practice collaborative version control._
...
```{r token generation, eval=FALSE, message=F, warning=F}
...
```{r cred, eval=FALSE, message=F, warning=F}
...
```{r fig3 ,eval = TRUE, echo=FALSE, out.width = '40%',out.height='40%' ,fig.align = 'center', fig.cap='This is what the URL should look like'}
knitr::include_graphics('Picture3.png')
```

   
```{r cloneR, eval=F, class.source='klippy', message=FALSE,warning=FALSE}
...
> _Goal: Create your first `.Rmd` document, understand the structure, render it, and begin using it as your scientific notebook for this course._
...
```{r loading them libraries, eval=FALSE,results='hide'}
...
```{r ,echo=FALSE, eval=TRUE, out.width = '80%', fig.align = 'center', fig.cap = 'The `YAML` header'}
...
```{r ,echo=FALSE, out.width = '80%', fig.align = 'center', fig.cap = 'default code chunk settings'}
...
> _Goal: Understand what Docker is and how to run bioinformatics tools in isolated, reproducible environments._
...
>**Note: For this part, make sure that docker desktop is runnig. If not, select if from the application launcher.**

1. Open a terminal or shell in RStudio or your system

2. Pull the BUSCO image:

```{bash BUSCO1, class.source='klippy'}
docker pull ezlabgva/busco
```

3. Check that it works by running:

```{bash BUSCO2, class.source='klippy'}
